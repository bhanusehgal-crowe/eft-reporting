"""Reported EFTR ingestor — pandas-free."""
import json
import uuid
...
class ReportedIngestor(BaseIngestor):
    ...
    REPORTED_HASH_FIELDS = [
        "report_reference", "reported_transaction_id", "report_date",
        "reported_amount", "reported_currency", "direction",
    ]
    ...
    def _persist(self, rows: list[dict], source_file: Path) -> int:
        ...
        for row in rows:
            ...
            rt = ReportedTransaction(
                ...
                row_hash=row_hash,
                report_reference=str(row["report_reference"]),
                reporting_entity_id=str(row["reporting_entity_id"]),
                reported_transaction_id=str(row["reported_transaction_id"]),
                report_date=row["report_date"],
                reported_amount=Decimal(str(row["reported_amount"])),
                reported_currency=str(row["reported_currency"]).upper(),
                reported_cad_amount=(
                    Decimal(str(row["reported_cad_amount"])) if row.get("reported_cad_amount") else None
                ),
